...
@section('title', __('User Identity Verify Confirm'))
@section('style')
                ...
                <div class="customMarkup__single">
                    <div class="customMarkup__single__item">
                        <div class="customMarkup__single__item__flex">
                            <h4 class="customMarkup__single__title">{{ __('User Identity Verify Confirm') }}</h4>
                        </div>
                        ...
                        <div class="customMarkup__single__inner mt-4">
                            <form action="{{route('admin.user.identity.verify.confirm')}}" method="POST" enctype="multipart/form-data">
                                @csrf
                                <x-form.text
                                    :title="__('Email Subject')"
                                    :type="__('text')"
                                    :name="'user_identity_verify_confirm_subject'"
                                    :id="'user_identity_verify_confirm_subject'"
                                    :value="get_static_option('user_identity_verify_confirm_subject') ?? __('User Identity Verify Confirm')"
                                />
                                <x-form.summernote
                                    :title="__('Email Message')"
                                    :name="'user_identity_verify_confirm_message'"
                                    :id="'user_identity_verify_confirm_message'"
                                    :value="get_static_option('user_identity_verify_confirm_message') ?? ''"
                                />

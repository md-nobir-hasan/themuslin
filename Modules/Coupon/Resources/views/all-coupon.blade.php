                        ...
                        <div class="dashboard__card__body custom__form mt-4">
                            <form action="{{ route('admin.product.coupon.new') }}" method="post"
                                ...
                                <button type="submit" id="coupon_create_btn"
                                    class="cmn_btn btn_bg_profile">{{ __('Add New Coupon') }}</button>
                            </form>
                        </div>
                    </div>
                </div>
        ...
        <div class="modal fade" id="category_edit_modal" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content custom__form">
                    ...
                    <form action="{{ route('admin.product.coupon.update') }}" method="post">
...
@endsection
@section('script')
    <script src="{{ asset('assets/tenant/frontend/js/jquery.nice-select.js') }}"></script>
    <x-datatable.js />
    <x-table.btn.swal.js />
    <x-bulk-action.js :route="route('admin.product.coupon.bulk.action')" />
    ...
    <script>
        ...
        function loadProductDiscountHtml(context, target_selector, is_edit, values) {
            ...
            if ($(context).val() == 'category') {
                ...
                $('.lds-ellipsis').fadeIn();
                $.get('{{ route('admin.product.coupon.products') }}').then(function(data) {
        ...
        function validateCoupon(context) {
            ...
            if (code.length) {
                ...
                $.get("{{ route('admin.product.coupon.check') }}", {
                    code: code
                }).then(function(data) {
                    if (data > 0) {
